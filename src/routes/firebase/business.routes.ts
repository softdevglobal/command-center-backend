...
import { getBusinessById, listBusinesses } from "../../services/business.service.js";
...
/**
 * GET /api/businesses
 * All registered business documents from Firestore `businesses` (bmspro-trade).
...
router.get("/", async (req, res) => {
  ...
  try {
    const result = await listBusinesses(options);
    res.json({
      success: true,
      ...result,
      ...authExtras(res),
 ...
 * One registered business document from Firestore `businesses` (bmspro-trade).
 */
router.get("/:id", async (req, res) => {
  ...
  try {
    const row = await getBusinessById(id);
    if (!row) {
    ...
    const msg = e instanceof Error ? e.message : "Failed to load business";
    res.status(500).json({ success: false, error: msg });
  }
});

...
/**
 * Inserts one row into `public.agents` matching the table contract:
 * required enums `agent_role` / `agent_status`, non-null arrays and text fields.
 */
export async function registerAgentInSupabase(input: {
  supabaseUrl: string;
  ...
  if (agentType === "workshop" && !workshopOwnerUid) {
    throw new Error("workshopOwnerUid is required — choose a workshop from BMS.");
  }
  if (agentType === "workshop" && !workshopBranchId) {
    throw new Error("workshopBranchId is required — choose a workshop branch.");
  }
  if (
    agentType === "workshop" &&

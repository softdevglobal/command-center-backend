...
/** Booking docs store the customer number under any of these fields depending on origin. */
const PHONE_FIELDS = [
  "clientPhone",
  "customerPhone",
  "phone",
...
/** Keep response size sane — most recent bookings after sorting. */
...
/**
 * Sri Lankan phone variant expansion, e.g. "+94766524216" →
 * ["94766524216", "+94766524216", "0766524216", "766524216"].
 * Capped at 30 entries (Firestore `in` limit). Empty array when input has no digits.
 */
...
/**
 * Looks up root `bookings` docs on bmspro-black matching any phone variant on any of the
 * known phone fields. Owner filtering happens in memory after the phone queries, so no
 * composite Firestore index is required. Most recent first, capped at 200.
 */
  ...
  await Promise.all(
    PHONE_FIELDS.map(async (field) => {
      try {
        ...
        // Some phone fields may not exist or be queryable — skip silently.
      }
    })
  );
